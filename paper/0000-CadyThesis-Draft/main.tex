...
\todo{general GOP formulation by witt, put this somewhere useful}
...
\end{equation}
where \[O\] is the segment, and \[NF(O)\] is the number of frames of segment \[O\].
Although \gls{gop} and other posterior-based scores continue to be used for \gls{md} with modern \gls{asr} architectures \parencite[inter alia]{parikhEvaluatingLogitBasedGOP2025,gongTransformerBasedMultiAspectMultiGranularity2022}, with the recent advent of transformer-based \gls{e2e} models, researchers have also pursued sequence alignment as an alternative to circumvent the need for phone-level forced-alignment of speech signals to reference transcriptions \parencite{loEffectiveEndtoEndModeling2020,leungCNNRNNCTCBasedEndtoend2019}. 
...
Following the general trend in \gls{asr} and \gls{ml} more broadly, \todo{not done. what did I want to do for this section again? if we do GOP, maybe shift this to GOP adaptions to CTC-trained end-to-end models}
...
\end{figure}
\todo{not sure if I need this figure still. keep it until it can't be justified, Qian et al., 2010; Li et al., 2017; Leung et al., 2019 find entries}
...
\numberedparagraph{more diverse tts generation a la l2gen}
Though we used the full array of available voices in the English cloud-based \gls{tts} system when synthesizing speech, we recognize that diversity of speech signals is key to producing  \gls{dpad}
